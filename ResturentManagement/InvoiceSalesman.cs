    ...
    public partial class InvoiceSalesman : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (IsValid())
            {
                ...
                SqlCommand cmd = new SqlCommand("INSERT INTO invoiceTable VALUES(@fooditem,@foodprice,@quantity,@totalprice,@discount,@after_discount)", con);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (foodId > 0)
            {
                SqlCommand cmd = new SqlCommand("UPDATE invoiceTable SET fooditem=@fooditem,quantity=@quantity WHERE id=@id", con);
                cmd.CommandType = CommandType.Text;
                cmd.Parameters.AddWithValue("@fooditem", textFoodName.Text);
                cmd.Parameters.AddWithValue("@quantity", textFoodQuantity.Text);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (foodId > 0)
            {
                ...
                SqlCommand cmd = new SqlCommand("DELETE FROM invoiceTable WHERE id=@id", con);
                cmd.CommandType = CommandType.Text;
