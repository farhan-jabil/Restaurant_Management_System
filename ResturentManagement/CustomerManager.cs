...
namespace ResturentManegment
{
    public partial class CustomerManager : Form
    {
        ...
        private void GetCustomersRecord()
        {
            ...
            SqlCommand cmd = new SqlCommand("select * from CustomerResturent",con);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (customerId > 0)
            {
                    SqlCommand cmd = new SqlCommand("UPDATE CustomerResturent SET name=@name,address=@address,mobile=@mobile WHERE customerid=@customerid", con);
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.AddWithValue("@name", textCustomerName.Text);
                    cmd.Parameters.AddWithValue("@address", textCustomerAddress.Text);
                    cmd.Parameters.AddWithValue("@mobile", textCustomerMobile.Text);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (customerId > 0)
            {
                ...
                SqlCommand cmd = new SqlCommand("DELETE FROM CustomerResturent WHERE customerid=@customerid", con);
                cmd.CommandType = CommandType.Text;
